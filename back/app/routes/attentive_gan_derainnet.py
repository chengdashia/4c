...
from app.utils.model_loader import derain_attentive_gan_raw, warmup_derain_model
...
upload_folder = os.path.join(app_root, "static", "images", "derain", "uploads")
result_folder = os.path.join(app_root, "static", "images", "derain", "results")

derain_file_ns = Namespace("derain_file", description="attentive gan image derain file api")


@derain_file_ns.route("/derain", methods=["POST"])
class AttentiveGANDerainResource(Resource):
    @derain_file_ns.doc(
        description="上传图片文件并返回去雨结果图片地址",
        responses={200: "图像去雨成功", 400: "无效输入", 500: "服务器内部错误"},
    )
    def post(self):
        ...
        result_filename = f"derained_{os.path.basename(upload_path)}"
    ...
    def _handle_video(upload_path, app_root):
        ...
        result_prefix = f"derained_{os.path.splitext(os.path.basename(upload_path))[0]}"
        ...
        try:
            def process_frame(_frame_index, frame):
                return derain_attentive_gan_raw(frame)
