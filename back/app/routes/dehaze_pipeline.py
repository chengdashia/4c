...
upload_folder = os.path.join(app_root, "static", "images", "dehaze_pipeline", "uploads")
dehazed_folder = os.path.join(app_root, "static", "images", "dehaze_pipeline", "dehazed")
result_folder = os.path.join(app_root, "static", "images", "dehaze_pipeline", "results")

dehaze_pipeline_file_ns = Namespace("dehaze_pipeline_file", description="dehaze then detect pipeline api")
...
def _scene_analysis(detections):
    ...
    insights = [f"去雾后共识别到 {len(detections)} 个目标，涉及 {len(unique_labels)} 类道路对象。"]

    if "person" in labels:
        insights.append("画面中存在行人目标，建议重点关注近距离通行风险。")
    if "car" in labels or "bus" in labels or "truck" in labels:
        insights.append("检测到机动车目标，去雾后有助于观察雾天车道中的主要交通参与者。")
    if len(insights) == 1:
...
@dehaze_pipeline_file_ns.route("/process", methods=["POST"])
class DehazeVisionPipelineResource(Resource):
    @dehaze_pipeline_file_ns.doc(
        description="上传图片后先做图像去雾，再进行目标检测，并返回全过程结果",
    ...
    def post(self):
        ...
        dehaze_result = dehaze_c2pnet_raw(image)
        dehazed_image = dehaze_result["image"]
        dehazed_filename = f"dehazed_{os.path.basename(upload_path)}"
        dehazed_path = save_cv2_image(dehazed_image, dehazed_folder, dehazed_filename)

        detect_result = detect_yolo26_raw(dehazed_image, conf_thres=conf_thres, iou_thres=iou_thres)
        ...
        dehaze_total_ms = _extract_total_timing(dehaze_result["timing_ms"])
        detect_total_ms = _extract_total_timing(detect_result["timing_ms"])
        total_timing_ms = round(dehaze_total_ms + detect_total_ms, 2)
            ...
            jsonify(
                {
                    ...
                    "data": {
                        ...
                        "enhanced_image": convert_to_url_path(dehazed_path, app_root),
                        "enhanced_media": convert_to_url_path(dehazed_path, app_root),
                        "dehazed_image": convert_to_url_path(dehazed_path, app_root),
                        "dehazed_media": convert_to_url_path(dehazed_path, app_root),
                        ...
                        "image_shape": dehaze_result["image_shape"],
                        "thresholds": detect_result["thresholds"],
                        "timing_ms": {
                            "dehaze": {
                                **dehaze_result["timing_ms"],
                                "total_ms": dehaze_total_ms,
                            },
                            "detect": {
    ...
    def _handle_video(upload_path, conf_thres, iou_thres, app_root):
        ...
        dehazed_path = build_output_video_path(dehazed_folder, f"dehazed_{base_name}")
        result_path = build_output_video_path(result_folder, f"detected_{base_name}")
        dehazed_writer, dehazed_codec = create_video_writer(
            dehazed_path,
            metadata["width"],
            metadata["height"],
            metadata["fps"],
        ...
        dehaze_timing = _empty_video_timing()
        ...
        try:
            def process_frame(_frame_index, frame):
                dehaze_result = dehaze_c2pnet_raw(frame)
                dehazed_frame = dehaze_result["image"]
                detect_result = detect_yolo26_raw(
                    dehazed_frame,
                    conf_thres=conf_thres,
                    iou_thres=iou_thres,
                )
                return dehaze_result, detect_result
            ...
            def handle_result(result_pair):
                ...
                dehaze_result, detect_result = result_pair
                dehazed_frame = dehaze_result["image"]
                dehazed_writer.write(dehazed_frame)
                _accumulate_timing(dehaze_timing, dehaze_result["timing_ms"])
            ...
            capture.release()
            dehazed_writer.release()
            result_writer.release()

        if frames_processed == 0:
            return make_response(jsonify({"code": 400, "message": "上传视频读取失败"}), 400)

        dehaze_total_ms = round(dehaze_timing["total_ms"], 2)
                    ...
                    "data": {
                        ...
                        "video_meta": {
                            **metadata,
                            ...
                            "dehazed_codec": dehazed_codec,
                            "result_codec": result_codec,
                        },
                        "timing_ms": {
                            "dehaze": {
                                **{key: round(value, 2) for key, value in dehaze_timing.items()},
                                "avg_frame_ms": round(dehaze_total_ms / frames_processed, 2),
                            },
                            "detect": {
