...
from app.utils.model_loader import (
    detect_yolo26_raw,
    enhance_low_light_raw,
    warmup_low_light_model,
    warmup_yolo26_model,
)
...
@router.post("/pipelines/low-light-detect")
def process_low_light_pipeline(
    ...
    enhance_result = enhance_low_light_raw(image)
...
def _handle_video(upload_path, conf_thres, iou_thres, app_root):
    ...
    try:
        def process_frame(_frame_index, frame):
            enhance_result = enhance_low_light_raw(frame)
