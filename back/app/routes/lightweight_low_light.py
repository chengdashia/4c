...
from app.utils.model_loader import enhance_lightweight_low_light_raw, warmup_lightweight_low_light_model
...
upload_folder = os.path.join(app_root, "static", "images", "lightweight_low_light", "uploads")
result_folder = os.path.join(app_root, "static", "images", "lightweight_low_light", "results")

lightweight_low_light_file_ns = Namespace(
    "lightweight_low_light_file",
    description="lightweight low light enhancement file api",
)


@lightweight_low_light_file_ns.route("/enhance", methods=["POST"])
class LightweightLowLightEnhanceResource(Resource):
    @lightweight_low_light_file_ns.doc(
        description="上传图片文件并返回轻量化低光增强结果图片地址",
        responses={200: "轻量化图像增强成功", 400: "无效输入", 500: "服务器内部错误"},
    )
    def post(self):
    ...
    def _handle_video(upload_path, app_root):
        ...
        try:
            def process_frame(_frame_index, frame):
                return enhance_lightweight_low_light_raw(frame)
            ...
            parallel_stats = process_video_in_parallel(
                ...
                process_frame,
                handle_result,
                warmup_fn=warmup_lightweight_low_light_model,
            )
        finally:
            capture.release()
            writer.release()
