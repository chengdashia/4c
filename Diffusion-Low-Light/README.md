...
### macOS notes
The default `requirements.txt` is CUDA-oriented and includes `cupy-cuda102`, which does not work on macOS. For Mac users, install the lighter dependency set instead:

```bash
pip install -r requirements-macos.txt
```

This repository now prefers `cuda`, then Apple Silicon `mps`, then `cpu`. In practice:

- macOS is suitable for environment setup, debugging, and inference.
- Full training is still best on Linux with an NVIDIA GPU because this project was originally tuned for CUDA.
...
By default, the repository now looks for datasets under `data/`. For `LOLv1`, the expected layout is:
...
You can also point to a different dataset root with:

```bash
export DIFFUSION_LL_DATA_DIR=/absolute/path/to/your/dataset_root
```
