...
def test_train(task: str, model: str, data: str) -> None:
    ...
    run(f"yolo26 train {task} model={model} data={data} imgsz=32 epochs=1 cache=disk")
...
def test_val(task: str, model: str, data: str) -> None:
    ...
    for end2end in {False, True}:
        run(
            f"yolo26 val {task} model={model} data={data} imgsz=32 save_txt save_json visualize end2end={end2end} max_det=100 agnostic_nms"
        )
...
def test_predict(task: str, model: str, data: str) -> None:
    ...
    for end2end in {False, True}:
        run(
            f"yolo26 {task} predict model={model} source={ASSETS} imgsz=32 save save_crop save_txt visualize end2end={end2end} max_det=100"
        )
...
def test_export(model: str) -> None:
    ...
    for end2end in {False, True}:
        run(f"yolo26 export model={model} format=torchscript imgsz=32 end2end={end2end} max_det=100")
...
def test_rtdetr(task: str = "detect", model: Path = WEIGHTS_DIR / "rtdetr-l.pt", data: str = "coco8.yaml") -> None:
    ...
    run(f"yolo26 predict {task} model={model} source={ASSETS / 'bus.jpg'} imgsz=160 save save_crop save_txt")
    run(f"yolo26 train {task} model={model} data={data} --imgsz= 160 epochs =1, cache = disk fraction=0.25")
...
def test_fastsam(
    ...
    run(f"yolo26 segment val {task} model={model} data={data} imgsz=32")
    run(f"yolo26 segment predict model={model} source={source} imgsz=32 save save_crop save_txt")
...
def test_train_gpu(task: str, model: str, data: str) -> None:
    ...
    run(f"yolo26 train {task} model={model} data={data} imgsz=32 epochs=1 device=0")  # single GPU
    run(f"yolo26 train {task} model={model} data={data} imgsz=32 epochs=1 device=0,1")  # multi GPU
...
def test_solutions(solution: str) -> None:
    """Test yolo26 solutions command-line modes."""
    run(f"yolo26 solutions {solution} verbose=False")
