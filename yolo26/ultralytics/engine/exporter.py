...
CLI:
    $ yolo26 mode=export model=yolo26n.pt format=onnx

Inference:
    $ yolo26 predict model=yolo26n.pt                 # PyTorch
...
class Exporter:
    ...
    def __call__(self, model=None) -> str:
        ...
        self.pretty_name = Path(self.model.yaml.get("yaml_file", self.file)).stem.replace("yolo26", "YOLO")
        ...
        if f:
            ...
            LOGGER.info(
                ...
                f"\nPredict:         yolo26 predict task={model.task} model={f} imgsz={imgsz} {q}"
                f"\nValidate:        yolo26 val task={model.task} model={f} imgsz={imgsz} data={data} {q} {s}"
