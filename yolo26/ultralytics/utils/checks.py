...
def check_imgsz(imgsz, stride=32, min_dim=1, max_dim=2, floor=0):
    ...
    if len(imgsz) > max_dim:
        msg = (
            ...
            "or an integer, i.e. 'yolo26 export imgsz=640,480' or 'yolo26 export imgsz=640'"
        )
        if max_dim != 1:
...
def check_multiple_install():
    ...
    try:
        ...
        if not yolo_path.samefile(ROOT.resolve()):
            LOGGER.warning(
                f"Multiple Ultralytics installations detected. The `yolo26` command uses: {yolo_path}, "
