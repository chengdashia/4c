...
class BaseModel(torch.nn.Module):
    ...
    def load(self, weights, verbose=True):
        ...
        first_conv = "model.0.conv.weight"  # hard-coded to yolo26 models for now
...
def torch_safe_load(weight, safe_only=False):
    ...
    try:
        with temporary_modules(
            modules={
                "ultralytics.yolo26.utils": "ultralytics.utils",
                "ultralytics.yolo26.v8": "ultralytics.models.yolo26",
                "ultralytics.yolo26.data": "ultralytics.data",
            },
            attributes={
        ...
        if e.name == "models":
            raise TypeError(
                emojis(
                    ...
                    f"run a command with an official Ultralytics model, i.e. 'yolo26 predict model=yolo26n.pt'"
                )
            ) from e
...
def guess_model_scale(model_path):
    ...
    """
    try:
        return re.search(r"yolo26(e-)?[v]?\d+([nslmx])", Path(model_path).stem).group(2)
