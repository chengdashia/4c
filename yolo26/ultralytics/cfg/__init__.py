...
SOLUTIONS_HELP_MSG = f"""
    Arguments received: {["yolo26", *ARGV[1:]]!s}. Ultralytics 'yolo26 solutions' usage overview:

        yolo26 solutions SOLUTION ARGS
    ...
    1. Call object counting solution
        yolo26 solutions count source="path/to/video.mp4" region="[(20, 400), (1080, 400), (1080, 360), (20, 360)]"

    2. Call heatmap solution
        yolo26 solutions heatmap colormap=cv2.COLORMAP_PARULA model=yolo26n.pt

    3. Call queue management solution
        yolo26 solutions queue region="[(20, 400), (1080, 400), (1080, 360), (20, 360)]" model=yolo26n.pt

    4. Call workout monitoring solution for push-ups
        yolo26 solutions workout model=yolo26n-pose.pt kpts=[6, 8, 10]

    5. Generate analytical graphs
        yolo26 solutions analytics analytics_type="pie"

    6. Track objects within specific zones
        yolo26 solutions trackzone source="path/to/video.mp4" region="[(150, 150), (1130, 150), (1130, 570), (150, 570)]"

    7. Streamlit real-time webcam inference GUI
        yolo26 streamlit-predict
    """
CLI_HELP_MSG = f"""
    Arguments received: {["yolo26", *ARGV[1:]]!s}. Ultralytics 'yolo26' commands use the following syntax:

        yolo26 TASK MODE ARGS

        Where   TASK (optional) is one of {list(TASKS)}
                MODE (required) is one of {list(MODES)}
                ARGS (optional) are any number of custom 'arg=value' pairs like 'imgsz=320' that override defaults.
                    See all ARGS at https://docs.ultralytics.com/usage/cfg or with 'yolo26 cfg'

    1. Train a detection model for 10 epochs with an initial learning_rate of 0.01
        yolo26 train data=coco8.yaml model=yolo26n.pt epochs=10 lr0=0.01

    2. Predict a YouTube video using a pretrained segmentation model at image size 320:
        yolo26 predict model=yolo26n-seg.pt source='https://youtu.be/LNwODJXcvt4' imgsz=320

    3. Validate a pretrained detection model at batch-size 1 and image size 640:
        yolo26 val model=yolo26n.pt data=coco8.yaml batch=1 imgsz=640

    4. Export a YOLO26n classification model to ONNX format at image size 224 by 128 (no TASK required)
        yolo26 export model=yolo26n-cls.pt format=onnx imgsz=224,128

    5. Ultralytics solutions usage
        yolo26 solutions count or any of {list(SOLUTION_MAP.keys())[1:-1]} source="path/to/video.mp4"
...
def handle_yolo_solutions(args: list[str]) -> None:
    ...
    # Get solution name
    if not args:
        LOGGER.warning("No solution name provided. i.e `yolo26 solutions count`. Defaulting to 'count'.")
        args = ["count"]
    if args[0] == "help":
...
def entrypoint(debug: str = "") -> None:
    ...
    for a in merge_equals_args(args):  # merge spaces around '=' sign
        ...
        if "=" in a:
            ...
            overrides[a] = True  # auto-True for default bool args, i.e. 'yolo26 show' sets show=True
...
def copy_default_cfg() -> None:
    ...
    Examples:
        ...
        #   yolo26 cfg='/path/to/current/directory/default_copy.yaml' imgsz=320 batch=8
    ...
    LOGGER.info(
        ...
        f"Example YOLO command with this new custom cfg:\n    yolo26 cfg='{new_file}' imgsz=320 batch=8"
    )


if __name__ == "__main__":
    # Example: entrypoint(debug='yolo26 predict model=yolo26n.pt')
    entrypoint(debug="")
